...
from lib.jamuna_tv import JamunaTvScraper
...
if __name__ == "__main__":
    ...
    if choice == '2':
        curr_date = generate_published_date()
        print(curr_date.get('readable'))
        create_dir_if_not_exists("data/jamunatv")

        if dir_exists(f'data/jamunatv/{curr_date['readable']}.txt'):
            cprint("Looks like you already scraped jamunatv today.", "red")
            cprint(
                f'Please delete the file: "data/jamunatv/{curr_date['readable']}.txt" before retrying',
                "red",
            )
